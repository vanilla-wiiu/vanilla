...
void handle_generic_packet(gamepad_context_t *info, int skt, GenericPacket *request)
{
    ...
    case SERVICE_ID_PERIPHERAL:
        ...
        case METHOD_ID_PERIPHERAL_NFC_CONTROL:
        {
            uint16_t response_size;
            gamepad_nfc_control(request->payload, request->generic_cmd_header.payload_size, response.payload, &response_size);
            response.generic_cmd_header.payload_size = htons(response_size);
            response.generic_cmd_header.error_code = 0;
            break;
        }
        case METHOD_ID_PERIPHERAL_TV_REMOTE_TEST_CODESET_SET:
        {
