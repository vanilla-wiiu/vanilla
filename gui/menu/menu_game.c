...
void show_error(vui_context_t *vui, void *v)
{
    ...
    vui_label_create(vui, 0, scrh * 1 / 6, scrw, scrh, lang(VPI_LANG_ERROR), vui_color_create(0.25f, 0.25f, 0.25f, 1),
                     VUI_FONT_SIZE_NORMAL, layer);
    ...
    vui_button_create(vui, scrw / 2 - ok_btn_w / 2, scrh * 3 / 4, ok_btn_w, BTN_SZ, lang(VPI_LANG_OK_BTN), 0,
                      VUI_BUTTON_STYLE_BUTTON, layer, back_to_main_menu, (void *) (intptr_t) layer);
...
size_t build_avcc(uint8_t *dst, size_t cap, const uint8_t *const *sps_list, const uint16_t *sps_len, int sps_count,
...
static int is_v4l2request_available(void)
{
    ...
    for (int i = 0; i < 64; i++) {
        ...
        for (int t = 0; t < 2; t++) {
            ...
            desc.type = (t == 0) ? V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE : V4L2_BUF_TYPE_VIDEO_OUTPUT;
...
int open_decoder(vpi_decode_state_t *s, hwdec_t *dec)
{
    ...
    if (BUILD_AVCC) {
        ...
        s->codec_ctx->extradata_size =
            build_avcc(s->codec_ctx->extradata, 1024, &p_sps, &sps_size, 1, &p_pps, &pps_size, 1, 4);
    }
...
int vpi_decode_init(vpi_decode_state_t *s)
{
    ...
    if (!vpi_config.force_software_decode) {
        ...
        if (r != VANILLA_SUCCESS && is_v4l2request_available()) {
            vpi_decode_exit(s);
            if ((ffmpeg_err = av_hwdevice_ctx_create(&s->hw_device_ctx, AV_HWDEVICE_TYPE_V4L2REQUEST, NULL, NULL, 0)) >=
                0) {
        ...
        if (r != VANILLA_SUCCESS) {
            vpi_decode_exit(s);
            if ((ffmpeg_err =
                     av_hwdevice_ctx_create(&s->hw_device_ctx, AV_HWDEVICE_TYPE_DRM, "/dev/dri/card0", 0, 0)) >= 0) {
...
int dump_frame_to_file(const AVFrame *frame, const char *filename)
{
    ...
    struct SwsContext *osws_ctx =
        sws_getContext(frame->width, frame->height, frame->format, frame->width, frame->height, opix_fmt, 0, 0, 0, 0);
    if (!osws_ctx) {
    ...
    ret = sws_scale(osws_ctx, (const uint8_t **) frame->data, (const int *) frame->linesize, 0, frame->height,
...
static int vpi_decode_drain(vpi_decode_state_t *s, size_t *outstanding_packets)
{
...
static int vpi_decode_submit_packet(vpi_decode_state_t *s, AVPacket *pkt, size_t *outstanding_packets)
{
...
static int vpi_decode_enqueue(vpi_decode_state_t *s, const uint8_t *data, size_t size)
{
    // Allocate new packet
...
void vpi_menu_game_start(vui_context_t *vui, void *v)
{
    ...
    if (r != VANILLA_SUCCESS) {
        ...
        int scrw, scrh;
        vui_get_screen_size(vui, &scrw, &scrh);
        status_lbl = vui_label_create(vui, 0, scrh * 2 / 5, scrw, scrh, buf, vui_color_create(0.5f, 0.5f, 0.5f, 1),
                                      VUI_FONT_SIZE_NORMAL, fglayer);

        // TODO: `state` does not get freed if
        const int cancel_btn_w = BTN_SZ * 3;
        int cancel_btn = vui_button_create(vui, scrw / 2 - cancel_btn_w / 2, scrh * 3 / 5, cancel_btn_w, BTN_SZ,
                                           lang(VPI_LANG_CANCEL_BTN), 0, VUI_BUTTON_STYLE_BUTTON, fglayer,
                                           cancel_connect, (void *) (intptr_t) fglayer);
