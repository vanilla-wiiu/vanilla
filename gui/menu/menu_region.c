...
void vpi_menu_region(vui_context_t *vui, void *v)
{
    ...
    vui_label_create(vui, bkg_rect.x + lbl_margin, lbl_y, bkg_rect.w - lbl_margin - lbl_margin,
                     bkg_rect.h - lbl_margin - lbl_margin, lang(VPI_LANG_REGION_HELP_1), vui_color_create(1, 1, 1, 1),
                     VUI_FONT_SIZE_NORMAL, fglayer);

    const int lbl2_width = bkg_rect.w * 2 / 3;
    vui_label_create(vui, bkg_rect.x + bkg_rect.w / 2 - lbl2_width / 2, lbl_y + bkg_rect.h / 8, lbl2_width,
                     bkg_rect.h - lbl_margin - lbl_margin, lang(VPI_LANG_REGION_HELP_2),
    ...
    for (int i = 0; i < MAX_REGIONS; i++) {
        int b = vui_button_create(vui, btn_x, btn_y + BTN_SZ * i, btn_w, BTN_SZ, lang(VPI_LANG_REGION_JAPAN + i), 0,
                                  VUI_BUTTON_STYLE_BUTTON, fglayer, region_clicked, (void *) (intptr_t) i);
