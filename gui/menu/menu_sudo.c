...
void vpi_menu_sudo(vui_context_t *vui, vui_callback_t success_action, void *success_data, vui_callback_t cancel_action,
                   void *cancel_data, int fade_fglayer)
{
    vui_reset(vui);
    ...
    vui_label_create(vui, bkg_rect.x, bkg_rect.y + bkg_rect.h * 1 / 10, bkg_rect.w, bkg_rect.h,
                     lang(VPI_LANG_SUDO_HELP_1), vui_color_create(1, 1, 1, 1), VUI_FONT_SIZE_NORMAL, fglayer);
    vui_label_create(vui, bkg_rect.x, bkg_rect.y + bkg_rect.h * 3 / 10, bkg_rect.w, bkg_rect.h,
                     lang(VPI_LANG_SUDO_HELP_2), vui_color_create(1, 1, 1, 1), VUI_FONT_SIZE_SMALL, fglayer);

    const int textedit_w = bkg_rect.w / 2;
    const int textedit_h = vui_get_font_height(vui, VUI_FONT_SIZE_NORMAL);
    textedit = vui_textedit_create(vui, bkg_rect.x + bkg_rect.w / 2 - textedit_w / 2, bkg_rect.y + bkg_rect.h * 4 / 10,
                                   textedit_w, textedit_h, "", VUI_FONT_SIZE_NORMAL, 1, fglayer);
    ...
    err_lbl = vui_label_create(vui, bkg_rect.x, bkg_rect.y + bkg_rect.h * 11 / 20, bkg_rect.w, bkg_rect.h, "",
                               vui_color_create(1, 0, 0, 1), VUI_FONT_SIZE_TINY, fglayer);

    static const int ok_btn_w = BTN_SZ * 3;
    static const int cancel_btn_w = BTN_SZ * 3;

    remember_btn = vui_button_create(vui, bkg_rect.w / 2 - textedit_w / 2, bkg_rect.y + bkg_rect.h * 6 / 10, textedit_w,
                                     BTN_SZ * 3 / 4, lang(VPI_LANG_AUTH_DONT_ASK_AGAIN), 0, VUI_BUTTON_STYLE_BUTTON,
                                     fglayer, dont_ask_again_btn_pressed, 0);
    vui_button_update_checkable(vui, remember_btn, 1);

    ok_btn = vui_button_create(vui, bkg_rect.x + bkg_rect.w / 2 - (ok_btn_w + cancel_btn_w) / 2,
                               bkg_rect.y + bkg_rect.h * 3 / 4, ok_btn_w, BTN_SZ, lang(VPI_LANG_OK_BTN), 0,
                               VUI_BUTTON_STYLE_BUTTON, fglayer, submit_pw, 0);
    cancel_btn = vui_button_create(vui, bkg_rect.x + bkg_rect.w / 2 - (ok_btn_w + cancel_btn_w) / 2 + ok_btn_w,
                                   bkg_rect.y + bkg_rect.h * 3 / 4, cancel_btn_w, BTN_SZ, lang(VPI_LANG_CANCEL_BTN), 0,
                                   VUI_BUTTON_STYLE_BUTTON, fglayer, return_to_main, (void *) (intptr_t) fglayer);
