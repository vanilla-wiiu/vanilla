...
void vpi_menu_edit_nofade(vui_context_t *vui, void *v)
{
    ...
    vui_label_create(vui, bkg_rect.x, bkg_rect.y + bkg_rect.h * 1 / 5, bkg_rect.w, bkg_rect.h,
                     vpi_config.connected_console_entries[console].name, vui_color_create(1, 1, 1, 1),
                     VUI_FONT_SIZE_NORMAL, fglayer);

    const int btn_w = bkg_rect.w / 2;
    const int btn_y = bkg_rect.y + bkg_rect.h * 1 / 2;
    vui_button_create(vui, bkg_rect.x + bkg_rect.w / 2 - btn_w / 2, btn_y + BTN_SZ * 0, btn_w, BTN_SZ,
                      lang(VPI_LANG_RENAME), 0, VUI_BUTTON_STYLE_BUTTON, fglayer, rename_btn_pressed, v);
    vui_button_create(vui, bkg_rect.x + bkg_rect.w / 2 - btn_w / 2, btn_y + BTN_SZ * 1, btn_w, BTN_SZ,
                      lang(VPI_LANG_DELETE), 0, VUI_BUTTON_STYLE_BUTTON, fglayer, delete_btn_pressed, v);
